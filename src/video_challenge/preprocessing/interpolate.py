...
def interpolate(data_3d):
    ...
    args:
        data_3d (np.ndarray): A 3D array containing landmark coordinates with potential NaN values.

    returns:
        np.ndarray: A 3D array of the same shape with NaNs replaced by interpolated values.
    """
    ...
    for j in range(n_joints):
        for c in range(n_coords):
            ...
            if np.any(nans) and not np.all(nans):
                ...
                filled_data[nans, j, c] = np.interp(
                    nan_indices, valid_indices, y[~nans]
                )
